...
namespace CrabGui
{
    ...
    // 从文件中创建
    Window* FreeParser::createFromFile(Window* pWndParent, PCStr pszFileName)
    {
    ...
    // 从内存中创建
    Window* FreeParser::createFromMemory(Window* pWndParent, void* pData, UInt nDataSize)
    {
		Window* pWindow = 0;
    ...
    // 保存到文件
    Bool FreeParser::saveToFile(Window* pWindow, PCStr pszFileName)
    {
    ...
    // 创建图片接口
    Image* FreeParser::createImage()
    {
		Image* pImage = CrabNew(FreeImageImage)();
    ...
    // 销毁图片接口
    void FreeParser::destroyImage(Image* pImage)
    {
		CrabDelete(pImage);
    }
    ...
    // 创建字体接口
    Font* FreeParser::createFont()
    {
		Font* pFont = CrabNew(FreeTypeFont)();
    ...
    // 销毁图片接口
    void FreeParser::destroyFont(Font* pFont)
    {
		CrabDelete(pFont);
    }
	...
	Window* FreeParser::_importWindow(UInt& uLayer, System* pSystem, const String& strLine)
	{
		Window* pWindow = 0;
		uLayer = 0;
	...
	// 从窗体导出字符串
	void FreeParser::_exportWindow(Window* pWindow, UInt uLayer)
	{
		// 导出当前窗体
